    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				fixed leftPixel = tex2D(_MainTex, i.uv + float2(-size.x, 0)).a;
				fixed upPixel = tex2D(_MainTex, i.uv + float2(0, size.y)).a;
				fixed rightPixel = tex2D(_MainTex, i.uv + float2(size.x, 0)).a;
				fixed bottomPixel = tex2D(_MainTex, i.uv + float2(0, -size.y)).a;
